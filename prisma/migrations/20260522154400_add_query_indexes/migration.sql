...
CREATE INDEX IF NOT EXISTS "repositories_user_id_created_at_idx" ON "repositories"("user_id", "created_at" DESC);

-- AddIndex: repositories(user_id, url)
-- Optimises: createRepository duplicate check WHERE url + userId
CREATE INDEX IF NOT EXISTS "repositories_user_id_url_idx" ON "repositories"("user_id", "url");
...
CREATE INDEX IF NOT EXISTS "analysis_jobs_repository_id_user_id_created_at_idx" ON "analysis_jobs"("repository_id", "user_id", "created_at" DESC);

-- AddIndex: accounts(user_id, provider)
-- Optimises: auth COUNT checks WHERE userId + provider (runs on every login/signup)
CREATE INDEX IF NOT EXISTS "accounts_user_id_provider_idx" ON "accounts"("user_id", "provider");
...
CREATE INDEX IF NOT EXISTS "github_repos_user_id_enabled_repo_full_name_idx" ON "github_repos"("user_id", "enabled" DESC, "repo_full_name" ASC);

-- AddIndex: github_repos(user_id, installation_id)
-- Optimises: sync / delete WHERE userId AND installationId IS NOT NULL
CREATE INDEX IF NOT EXISTS "github_repos_user_id_installation_id_idx" ON "github_repos"("user_id", "installation_id");

-- AddIndex: branches(repository_id, is_default DESC)
-- Optimises: getRepository ORDER BY isDefault DESC scoped to a repo
CREATE INDEX IF NOT EXISTS "branches_repository_id_is_default_idx" ON "branches"("repository_id", "is_default" DESC);

-- AddIndex: file_changes(file_id)
-- Optimises: onDelete SetNull cascade lookups on file_id FK
CREATE INDEX IF NOT EXISTS "file_changes_file_id_idx" ON "file_changes"("file_id");

-- AddIndex: contributors(repository_id, commits DESC)
-- Optimises: getRepository ORDER BY commits DESC scoped to a repo
CREATE INDEX IF NOT EXISTS "contributors_repository_id_commits_idx" ON "contributors"("repository_id", "commits" DESC);
...
CREATE INDEX IF NOT EXISTS "languages_repository_id_percentage_idx" ON "languages"("repository_id", "percentage" DESC);
