...
export class AuthorizationAudit {
  ...
  public static log(entry: Omit<AuthorizationAuditEntry, "timestamp">): void {
    ...
    if (entry.success) {
      console.log(
        `[AUTHZ AUDIT] Success: ${entry.action.toUpperCase()} - User ID: ${entry.userId} - Repo ID: ${entry.repositoryId}`,
        JSON.stringify(payload),
      );
    } else {
      console.error(
        `[AUTHZ AUDIT] FAILURE: ${entry.action.toUpperCase()} - User ID: ${entry.userId} - Repo ID: ${entry.repositoryId} - Reason: ${entry.reason}`,
        JSON.stringify(payload),
      );
    }
  }
