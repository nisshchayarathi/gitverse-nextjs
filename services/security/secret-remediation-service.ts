...
export class SecretRemediationService {
  private static providerMap: Record<
    SecretProvider,
    { envVar: string; severity: "critical" | "high" | "medium" | "low" }
  > = {
  ...
  public static classify(
    ...
    if (/A3T[A-Z0-9]|AKIA[A-Z0-9]/i.test(secret)) {
      ...
      /ghp_[a-zA-Z0-9]{36}|github_pat_[a-zA-Z0-9_]{82}/i.test(secret)
    ) {
      provider = "GitHub";
      confidence = 0.98;
  ...
  public static async generateWorkflow(
    ...
    if (
      finding.filePath.endsWith(".yaml") ||
      finding.filePath.endsWith(".yml")
    ) {
    ...
    console.warn(
      `[SECURITY AUDIT] Remediation generated successfully for ${finding.provider} credential.`,
    );
