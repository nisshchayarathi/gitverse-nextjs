...
export class OrgRagIndex {
  ...
  async retrieveCrossRepositoryContext(
    repoFullName: string,
    query: string,
    maxFiles: number = 5,
  ): Promise<string[]> {
    console.log(
      `[OrgRagIndex] Retrieving cross-repo context for ${repoFullName}`,
    );

    const downstream = orgKnowledgeGraph.getDownstreamDependents(
      repoFullName,
      2,
    );
