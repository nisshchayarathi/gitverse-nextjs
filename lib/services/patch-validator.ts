...
export class PatchValidatorService {
  ...
  validatePatch(
    ...
    if (
      fullPatch.file.endsWith(".ts") ||
      fullPatch.file.endsWith(".tsx") ||
      fullPatch.file.endsWith(".js") ||
      fullPatch.file.endsWith(".jsx")
    ) {
      ...
      const hasErrors =
        sfAny.parseDiagnostics && sfAny.parseDiagnostics.length > 0;
      if (hasErrors) {
