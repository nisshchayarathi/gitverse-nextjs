...
export class CrossRepoImpactService {
  analyzeImpact(
    ...
    const repoDownstream = orgKnowledgeGraph.getDownstreamDependents(
      modifiedRepo,
      2,
    );
    ...
    if (
      modifiedFiles.some(
        (f) =>
          f.includes("types") || f.includes("interface") || f.includes("api"),
      )
    ) {
      reason =
        "Shared types or interfaces were modified, impacting downstream consumers.";
