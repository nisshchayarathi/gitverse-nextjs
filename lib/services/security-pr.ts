...
export class SecurityPRService {
  ...
  async runPipeline(
    ...
    for (const scanResult of vulnerableDeps) {
      ...
      const plan = await this.migrator.planAndExecuteMigration(
        repoPath,
        scanResult,
      );
      if (!plan) continue;
      ...
      // 3. Run Validation
      const validation = await this.validator.runValidation(
        repoPath,
        plan.refactoredFiles.length > 0,
      );
      ...
      reports.push({
        ...
        migrationConfidence:
          plan.refactoredFiles.length > 0
            ? plan.refactoredFiles.reduce(
                (acc, f) => acc + f.confidenceScore,
                0,
              ) / plan.refactoredFiles.length
  ...
  private async createSecurityPR(
    ...
    console.log(
      `[SecurityPR] Created branch ${branchName} and committed changes.`,
    );
