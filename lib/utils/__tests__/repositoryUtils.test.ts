...
describe("repositoryUtils", () => {
  ...
  describe("formatRelativeTime", () => {
    it("returns formatted relative times correctly", () => {
      const now = new Date();
      expect(formatRelativeTime(new Date(now.getTime() - 10 * 1000))).toBe(
        "Just now",
      );
      ...
      expect(
        formatRelativeTime(new Date(now.getTime() - 3 * 3600 * 1000)),
      ).toBe("3h ago");
    });
  });
  ...
  describe("generateAvatar", () => {
    it("returns a DiceBear avatar SVG URL based on seed", () => {
      expect(generateAvatar("test@example.com")).toContain(
        "seed=test%40example.com",
      );
    });
  });
  ...
  describe("normalizeKnownRepoHttpUrl", () => {
    it("standardizes supported provider repository URLs", () => {
      expect(
        normalizeKnownRepoHttpUrl("https://github.com/user/repo.git"),
      ).toBe("https://github.com/user/repo");
      expect(normalizeKnownRepoHttpUrl("https://gitlab.com/group/repo")).toBe(
        "https://gitlab.com/group/repo",
      );
      expect(
        normalizeKnownRepoHttpUrl("https://unsupported.com/user/repo"),
      ).toBeNull();
  ...
  describe("normalizeTargetDirectory", () => {
    it("normalizes target directory path strings", () => {
      ...
      expect(normalizeTargetDirectory("./src/components/")).toBe(
        "src/components",
      );
