...
export class ShardMigrationWorker {
  ...
  private updateJobState(
    ...
    if (job) {
      ...
      this.activeJobs.set(jobId, job);
      console.log(
        `[MigrationWorker] Job ${jobId} transitioned to ${state} (${percent}%)`,
      );
    }
  }
  ...
  private async processMigration(jobId: string): Promise<void> {
    ...
    try {
      const sourceClient = prismaShardRouter.getClientForShard(
        job.sourceShardId,
      );
      const targetClient = prismaShardRouter.getClientForShard(
        job.targetShardId,
      );
      ...
      if (!validation.isValid) {
        throw new Error(
          `Verification failed: ${validation.reasons.join(", ")}`,
        );
      }
