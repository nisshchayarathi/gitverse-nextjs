 ...
 *
 * NOTE: Mock session mechanisms that bypass authentication are intentionally
 * omitted. Any mechanism that allows bypassing auth in non-production
 * environments is a security risk if the bypass flag leaks to production.
 * Use real test accounts / NextAuth test adapters instead.
 */

export async function middleware(request: NextRequest) {
  const { pathname } = request.nextUrl;

  // Bypass middleware for all webhook routes.
  ...
  if (isProtectedRoute && !isAuthenticated) {
    ...
    // Include query parameters so deep links are not destroyed on redirect.
