...
export default function RepositoryKnowledgeSettings() {
  ...
  const handleRefresh = async () => {
    try {
      ...
      setError(null);
      const response = await fetch(
        `/api/repositories/${repositoryId}/knowledge/refresh`,
        {
          method: "POST",
        },
      );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            Repository Knowledge Settings
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Maintainer-defined context and AI onboarding glossaries injected
      ...
      Object.keys(knowledge).filter((k) => k !== "updatedAt").length === 0 ? (
        <div className="bg-white dark:bg-gray-800 p-8 text-center rounded-lg shadow border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-gray-500 max-w-lg mx-auto">
            To provide custom context to the AI, add a{" "}
            ...
            file to the root of your repository and refresh this page.
          </p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          {knowledge.glossary && Object.keys(knowledge.glossary).length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(knowledge.glossary).map(
                  ([term, definition]) => (
                    <div
                      key={term}
                      className="bg-gray-50 dark:bg-gray-900 p-4 rounded border border-gray-100 dark:border-gray-700"
                    >
          ...
          {knowledge.onboardingNotes &&
            knowledge.onboardingNotes.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
