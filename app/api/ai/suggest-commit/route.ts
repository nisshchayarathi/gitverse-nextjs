...
function validateArrayField(
  ...
  for (const item of items) {
    if (
      typeof item === "string" &&
      item.length > AI_REQUEST_LIMITS.MAX_ARRAY_ITEM_CHARS
    ) {
      return NextResponse.json(
        {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      diff &&
      typeof diff === "string" &&
      diff.length > AI_REQUEST_LIMITS.MAX_DIFF_CHARS
    ) {
      return NextResponse.json(
        {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "At least one of added, modified, deleted, or diff is required",
        },
        { status: 400 },
      );
    }
    ...
    logger.error(
      {
        ...
        route: "app/api/ai/suggest-commit/route.ts",
      },
