...
export class GraphAnalyzer {
  public buildDependencyGraph(files: RawFile[]): {
    nodes: GraphNode[];
    links: GraphLink[];
  } {
    ...
    const tree = new Map<
      string,
      { type: "folder" | "file"; children: Set<string>; fileRef?: RawFile }
    >();
    ...
    while (queue.length > 0) {
      ...
      if (path !== "root") {
        ...
        nodesMap.set(nodeId, {
          ...
          type: nodeData.type,
          size: nodeData.fileRef
            ? Math.min(Math.max((nodeData.fileRef.lines || 500) / 10, 40), 150)
            : 100,
          path,
          depth,
