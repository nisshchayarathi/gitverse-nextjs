...
export function MiniMap({
  ...
  useEffect(() => {
    ...
    const maxX =
      d3.max(nodes, (d) => (d.x as number) + (d.size || 10)) || width;
    const minY = d3.min(nodes, (d) => (d.y as number) - (d.size || 10)) || 0;
    const maxY =
      d3.max(nodes, (d) => (d.y as number) + (d.size || 10)) || height;
    ...
    const scale =
      Math.min(minimapWidth / graphWidth, minimapHeight / graphHeight) * 0.9;
