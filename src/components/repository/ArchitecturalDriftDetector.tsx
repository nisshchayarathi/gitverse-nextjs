...
} from "@/services/architectureDriftService";
import {
...
export function ArchitecturalDriftDetector({
  repository,
  loading = false,
}: ArchitecturalDriftDetectorProps) {
  const files = useMemo(
    () => (repository?.files || []) as RepositoryFile[],
    [repository?.files],
  );
  const [viewMode, setViewMode] = useState<"overview" | "comparison">(
    "overview",
  );
    ...
    return (
      <Card className="glass">
        <CardHeader>
          ...
          <CardDescription>
            Analyzing repository architecture and complexity trends.
          </CardDescription>
        </CardHeader>
        <CardContent>
    ...
    return (
      <Card className="glass">
        <CardHeader>
          <CardTitle>Architectural Drift Detector</CardTitle>
          <CardDescription>
            Track repository architecture evolution and complexity trends.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <EmptyState
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-amber-300">
                      {metrics.circulars} Circular Dependencies Detected
                    </p>
                    <p className="mt-1 text-sm text-amber-200">
                      Consider refactoring to break these cycles and improve
              ...
              <div className="space-y-2 text-sm text-foreground">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Indicates good module separation
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Measures internal connectivity
                  </span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="space-y-2 text-sm">
                {health.modularity < 50 && (
                  <div className="flex gap-2 text-amber-300">
                    <span>→</span>
                    <span>
                      Improve module boundaries to increase modularity
                    </span>
                  </div>
                )}
                {health.coupling > 60 && (
                  <div className="flex gap-2 text-amber-300">
                    <span>→</span>
                    <span>
                      High coupling detected. Consider introducing interfaces or
                      facade patterns
                    </span>
                  </div>
                )}
                {health.complexity > 70 && (
                  <div className="flex gap-2 text-amber-300">
                    <span>→</span>
                    <span>
                      Reduce complexity by breaking down large modules
                    </span>
                  </div>
                )}
                {health.health >= 80 && (
                  <div className="flex gap-2 text-emerald-300">
                    <span>✓</span>
                    <span>
                      Architecture is well-structured. Maintain current patterns
                    </span>
                  </div>
                )}
              </div>
      ...
      <CardFooter className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-foreground">
            Use these insights to guide refactoring and improvement initiatives.
          </p>
        </div>
