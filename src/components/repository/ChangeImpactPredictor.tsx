...
export function ChangeImpactPredictor({
  repository,
  selectedFile,
  loading,
}: ChangeImpactPredictorProps) {
    ...
    return (
      <Card className="glass">
        <CardHeader>
          ...
          <CardDescription>
            Evaluating file dependencies and risk score.
          </CardDescription>
        </CardHeader>
        <CardContent>
    ...
    return (
      <Card className="glass">
        <CardHeader>
          ...
          <CardDescription>
            Click a repository file to view its estimated impact.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <EmptyState
    ...
    return (
      <Card className="glass">
        <CardHeader>
          ...
          <CardDescription>
            Unable to compute impact for the selected file.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              The selected file metadata does not include enough dependency data
              to calculate risk.
            </p>
            <p className="text-sm text-muted-foreground">
              If your repository exposes import/export metadata or file content,
              this analysis will become more accurate.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <CardHeader>
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <CardDescription>
              Predicts the potential impact of modifying{" "}
              <span className="font-semibold">{selectedFile.path}</span>.
            </CardDescription>
          </div>
          <span
            className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${badgeStyles[impact.riskLevel]}`}
          >
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <p className="mt-3 text-3xl font-bold">
                  {impact.indirectDependencies.length}
                </p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs uppercase tracking-[0.12em] text-muted-foreground">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-3 space-y-2">
                  {impact.recommendedTests.map((test) => (
                    <div
                      key={test}
                      className="rounded-xl bg-slate-950/60 px-3 py-2 text-sm text-slate-100"
                    >
                      {test}
                    </div>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                This predictor is built with reusable dependency traversal logic
                and can be extended for PR-level impact predictions.
              </p>
            </div>
          </div>
        </div>
