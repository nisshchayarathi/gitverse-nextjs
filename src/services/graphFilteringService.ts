...
export class GraphFilteringService {
  private isHidden(path: string, options: GraphAnalyzerOptions): boolean {
    ...
    if (
      options.hiddenFileTypes &&
      options.hiddenFileTypes.length > 0 &&
      !path.endsWith("/")
    ) {
  ...
  public applyFilters(
    ...
    const rootNodes = completeNodes.filter(
      (n) => n.path.indexOf("/") === -1 && !this.isHidden(n.path, options),
    );
    if (
      rootNodes.length === 1 &&
      rootNodes[0].type === "folder" &&
      !options.expandedNodes.has(rootNodes[0].id)
    ) {
    ...
    for (const path of visiblePaths) {
      ...
      if (node) {
        const isExpanded =
          node.type === "folder" && options.expandedNodes.has(node.id);
