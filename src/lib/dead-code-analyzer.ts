...
export function analyzeDeadCode(
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (namedExport) {
        ...
        if (
          ...
          (file.path.includes("hooks") ||
            (name.startsWith("use") && name[3]?.match(/[A-Z]/)))
        ) {
          type = "hook";
        } else if (keyword === "class") {
          type = "service";
        }
        allExports.set(`${file.path}:${name}`, {
          file: file.path,
          line: i + 1,
          type,
          name,
      ...
      const defaultExport = line.match(
  ...
  for (const [key, info] of allExports) {
    ...
    const isImported = Array.from(allImports).some(
      (imp) => imp === name || imp.includes(name),
    );
    ...
    if (!isImported && !name.startsWith("_")) {
      ...
      if (
        fileIsConfig(info.file) ||
        fileIsTest(info.file) ||
        fileIsTypeDef(info.file)
      ) {
        confidence = "LOW";
        reason =
          "Configuration, test, or type definition file may not need direct imports";
      }

      if (info.type === "api-route") {
        confidence = "MEDIUM";
        reason =
          "API route may be consumed externally or by Next.js file conventions";
        suggestion =
          "Check if this route is called by external services or linked in navigation";
      }

      if (info.type === "page") {
        confidence = "LOW";
        reason =
          "Page components are often routed by the framework, not directly imported";
...
function fileIsConfig(filePath: string): boolean {
  return /\.config\.(ts|js|mjs)$|(next|tailwind|postcss|tsconfig|jest|vitest|playwright|eslint|prettier|babel|webpack|vite|rollup)/.test(
    filePath,
  );
}

function fileIsTest(filePath: string): boolean {
  return /\.(test|spec|e2e)\.(ts|tsx|js|jsx)$|__tests__|__mocks__/.test(
    filePath,
  );
}
...
export function getCleanupRecommendation(finding: DeadCodeFinding): string {
  const actions: Record<string, string[]> = {
    component: [
      ...
      "Inline the component if used in only one place",
    ],
    ...
    "api-route": [
      ...
      "Add a comment explaining external usage",
    ],
