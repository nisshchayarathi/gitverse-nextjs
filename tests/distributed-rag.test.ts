...
describe("Distributed RAG Indexing and Cross-Repo Analysis", () => {
  ...
  test("Scenario 1: Shared types -> frontend + backend", () => {
    ...
    orgKnowledgeGraph.addDependency({
      sourceRepo: "shared-types",
      targetRepo: "frontend-web",
      sourceFile: "User.ts",
    ...
    const report = crossRepoImpactService.analyzeImpact("shared-types", [
      "User.ts",
    ]);
  ...
  test("Scenario 2: SDK -> multiple services", () => {
    ...
    orgKnowledgeGraph.addDependency({
      sourceRepo: "core-sdk",
      targetRepo: "service-auth",
      sourceFile: "api.ts",
      dependencyType: "sdk",
    });
    orgKnowledgeGraph.addDependency({
      sourceRepo: "core-sdk",
      targetRepo: "service-billing",
      sourceFile: "api.ts",
      dependencyType: "sdk",
    });
    orgKnowledgeGraph.addDependency({
  ...
  test("Scenario 3: Breaking API contract change", () => {
    orgKnowledgeGraph.addDependency({
      ...
      sourceFile: "routes/user.ts",
      dependencyType: "api",
    });

    const report = crossRepoImpactService.analyzeImpact("backend-api", [
      "routes/user.ts",
    ]);
