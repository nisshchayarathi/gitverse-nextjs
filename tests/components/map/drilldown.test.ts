...
describe("GraphAnalyzer - Drilldown and Filtering", () => {
  ...
  it("Scenario 1: Expand module - only top level and expanded modules are visible", () => {
    ...
    const result1 = filterService.applyFilters(
      completeGraph.nodes,
      completeGraph.links,
      options1,
    );
    ...
    const result2 = filterService.applyFilters(
      completeGraph.nodes,
      completeGraph.links,
      options2,
    );
  ...
  it("Scenario 2: Collapse module - children hidden", () => {
    ...
    expect(result2.nodes.map((n) => n.id)).not.toContain(
  ...
  it("Scenario 3: Hide node_modules - removed from graph", () => {
    ...
    const result = filterService.applyFilters(
      completeGraph.nodes,
      completeGraph.links,
      options,
    );
