...
describe("Secret Exposure Engine", () => {
  it("detects high entropy strings", () => {
    const highEntropy =
      "aGVsbG8gd29ybGQgYW5kIG1vcmUgdGV4dCB0aGF0IGlzIGJhc2U2NCBlbmNvZGVkIQ==";
  ...
  it("detects AWS keys", async () => {
    ...
    const result = await secretDetector.scanFile(
      "config.js",
  ...
  it("detects Stripe keys", async () => {
    const result = await secretDetector.scanFile(
      "payment.ts",
      "const key = 'sk_test_abc1234567890def123456789'",
    );
    expect(result.length).toBeGreaterThan(0);
